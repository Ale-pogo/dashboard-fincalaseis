...
const sheetName = workbook.SheetNames.find((name) => name.toLowerCase().includes('semanal'));
console.log('sheetName', sheetName);
const worksheet = workbook.Sheets[sheetName];
const rows = XLSX.utils.sheet_to_json(worksheet, { header: 1, blankrows: false, defval: '' });
console.log('currentWeek', getCurrentWeekString());
console.log('first rows', rows.slice(0, 12));
